...
.task{{background:var(--panel);border:1px solid var(--line);padding:20px;margin-bottom:18px}}.task h2{{margin:0 0 9px}}
.prompt{{white-space:pre-wrap;border-left:4px solid var(--ochre);padding-left:14px}}.columns{{display:grid;grid-template-columns:1fr 1fr;gap:14px}}
.answer{{background:var(--panel);border:1px solid var(--line);overflow:hidden}}.answer h3{{margin:0;padding:12px 16px;background:#e8e4db;border-bottom:1px solid var(--line)}}
...
.score-card{{background:var(--panel);border:1px solid var(--line);border-top:3px solid var(--ochre);padding:16px}}label{{display:block;margin:9px 0;font-size:12px}}
